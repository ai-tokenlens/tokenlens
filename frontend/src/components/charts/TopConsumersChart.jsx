import {
  BarChart, Bar, XAxis, YAxis, Tooltip, ResponsiveContainer, Cell,
} from 'recharts'
import { useTopConsumers } from '../../api/client.js'
import { formatTokens } from '../../utils/formatters.js'

const COLORS = [
  '#6366f1','#8b5cf6','#a78bfa','#818cf8','#60a5fa',
  '#34d399','#fbbf24','#f87171','#fb923c','#e879f9',
]

export default function TopConsumersChart({ from, to }) {
  const { data, isLoading, isError } = useTopConsumers({ limit: 10, from, to })

  return (
    <div className="bg-white rounded-xl border border-gray-200 p-6">
      <h3 className="text-sm font-medium text-gray-500 mb-4">Top Consumers (tokens)</h3>

      {isLoading && <p className="text-sm text-gray-400 py-8 text-center">Loading…</p>}
      {isError && <p className="text-sm text-red-400 py-8 text-center">Failed to load data.</p>}
      {data && data.length === 0 && (
        <p className="text-sm text-gray-400 py-8 text-center">No data.</p>
      )}
      {data && data.length > 0 && (
        <ResponsiveContainer width="100%" height={260}>
          <BarChart
            data={data}
            layout="vertical"
            margin={{ left: 8, right: 16 }}
          >
            <XAxis
              type="number"
              tickFormatter={(v) => formatTokens(v)}
              tick={{ fontSize: 11 }}
            />
            <YAxis
              type="category"
              dataKey="user_id"
              width={100}
              tick={{ fontSize: 11 }}
            />
            <Tooltip formatter={(v) => [formatTokens(v), 'Tokens']} />
            <Bar dataKey="total_tokens" radius={[0, 4, 4, 0]}>
              {data.map((_, i) => (
                <Cell key={i} fill={COLORS[i % COLORS.length]} />
              ))}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
      )}
    </div>
  )
}
